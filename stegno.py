...
def img_ConvToPNG():
    ...
    s_path = str(l.get())
    d_path = os.path.join(str(m.get()), str(c.get()) + ".png")
    print(s_path)
    print(d_path)
...
def message(): #this function is use to convert text message into ascii- binary
    ...
    length=length[2:].zfill(10) #$$$$ by changing the value of Zfill, user that number of character
...
def oddConvt(value): #function converts even number into odd in given range
    value=value+1
    return max(0, min(225,value))


def encode(): #encode message in given image and return steganogrphy img
...
def decode():
    global dec_path
    dec_path = str(ds.get())
    print("Decoding from:", dec_path)  # Debugging statement
    try:
...
def encWin():
    ...
    Radiobutton(root, text="Video", variable=q, value=2, indicatoron=0).pack()
    ...
    label6 = Label(root, width=120,text="", font=(None,18)).pack()
    label7 = Label(root, width=120,text="MAJOR PROJECT_BATCH_85", font=(None,19)).pack()
    global inputtxt
...
def decWin():
    ...
    label7= Label(root2, text=" The decoded message will be opened in next window and is also saved in same directory as source")
    """ output = Text( root2, height=10, width=50).insert(END,asciiData)"""
...
# Define Decoded_message as a global variable
Decoded_message = ""
...
def decMessage():
    ...
    # Use the global Decoded_message variable here
...
dec_button=Button(parent, text="Decode", width=10, command=decWin).place(relx=0.5, rely=0.45, anchor=N)
